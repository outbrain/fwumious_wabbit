...
impl NamespaceTransformsParser {
    ...
    pub fn add_transform_namespace(
        ...
        let namespace_descriptor = vw
            .map_verbose_to_namespace_descriptor
            .get(&to_namespace_verbose);
        if namespace_descriptor.is_some() {
            return Err(Box::new(IOError::new(
                ErrorKind::Other,
                format!(
                    "To namespace of {} already exists as primitive namespace: {:?}",
                    s, to_namespace_verbose
                ),
            )));
        }

        self.denormalized.insert(
            to_namespace_verbose.to_owned(),
            NSStage1Parse {
    ...
    pub fn depth_first_search(
        ...
        let n = match self.denormalized.get(verbose_name) {
            ...
            None => {
                return Err(Box::new(IOError::new(
                    ErrorKind::Other,
                    format!("Could not find namespace {:?}", verbose_name),
                )))
            }
        };
        ...
        if n.processing.get() {
            return Err(Box::new(IOError::new(
                ErrorKind::Other,
                format!(
                    "Cyclic dependency detected, one of the namespaces involved is {:?}",
                    verbose_name
                ),
            )));
        }
...
impl NamespaceTransforms {
    ...
    fn add_transform(&mut self, vw: &vwmap::VwNamespaceMap, s: &str) -> Result<(), Box<dyn Error>> {
        ...
        let to_namespace_descriptor =
            get_namespace_descriptor_verbose(self, vw, &to_namespace_verbose);
        if to_namespace_descriptor.is_ok() {
            return Err(Box::new(IOError::new(
                ErrorKind::Other,
                format!(
                    "To namespace of {} already exists: {:?}",
                    s, to_namespace_verbose
                ),
            )));
        }
        ...
        for from_namespace_verbose in &from_namespaces_verbose {
            let from_namespace_descriptor =
                get_namespace_descriptor_verbose(self, vw, from_namespace_verbose)?;
            from_namespaces.push(Namespace {
                namespace_descriptor: from_namespace_descriptor,
...
pub fn get_namespace_descriptor_verbose(
        ...
        .get(namespace_verbose)
    {
        ...
        None => {
            ...
            let f: Vec<&NamespaceTransform> = transform_namespaces
                .v
                .iter()
            ...
            if f.is_empty() {
                return Err(Box::new(IOError::new(
                    ErrorKind::Other,
                    format!(
                        "Unknown verbose namespace in command line: {}",
                        namespace_verbose
                    ),
                )));
            } else {
...
pub fn parse_function_params_namespaces(input: &str) -> IResult<&str, Vec<String>> {
    ...
    let (input, (_, namespaces_str, _)) = tuple((
        take_open,
        nom::multi::separated_list1(take_separator, parse_identifier),
        take_close,
    ))(input)?;
...
pub fn parse_function_params_floats(input: &str) -> IResult<&str, Vec<f32>> {
    ...
    let (input, (_, namespaces_str, _)) = tuple((
        take_open,
        nom::multi::separated_list0(take_separator, parse_float),
        take_close,
    ))(input)?;
    Ok((input, namespaces_str))
}
...
pub fn parse_namespace_statement(
    input: &str,
) -> IResult<&str, (String, String, Vec<String>, Vec<f32>)> {
...
"#;
...

        {
            ...
            assert_eq!(
                nst.v[0].from_namespaces[1].namespace_descriptor,
                ns_desc_f32(1)
            );
        }

        {
            ...
            assert_eq!(
                format!("{:?}", result),
                "Err(Custom { kind: Other, error: \"Unknown transformer function: unknown\" })"
            );
        }
        ...
        {
            let mut nstp = NamespaceTransformsParser::new();
            nstp.add_transform_namespace(&vw, "new=unknown(nonexistent,featureB)()")
                .unwrap(); // unknown function
            let result = nstp.resolve(&vw);
            assert!(result.is_err());
...

        {
            ...
            assert_eq!(
                nst.v[1].from_namespaces[0].namespace_descriptor,
                ns_desc_trans(0)
            );
            assert_eq!(
                nst.v[1].from_namespaces[1].namespace_descriptor,
                ns_desc_f32(1)
            );
        }
