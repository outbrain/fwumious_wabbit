...
impl ExecutorToNamespace {
    ...
    #[inline(always)]
    pub fn emit_i32<const SEED_ID: usize>(&mut self, to_data: i32, hash_value: f32, bin_value: f32) {
        let hash_index = murmur3::hash32_with_seed(to_data.to_le_bytes(), *unsafe {
            self.namespace_seeds.get_unchecked(SEED_ID)
        }) & parser::MASK31;
        self.tmp_data.push((hash_index, hash_value, bin_value));
    }
    ...
    pub fn emit_f32<const SEED_ID: usize>(&mut self, f: f32, hash_value: f32, interpolated: bool) {
        if !f.is_finite() {
            ...
            self.emit_i32::<SEED_ID>(f.to_bits() as i32, hash_value, 0.0);
        } else {
            if interpolated {
                let floor = f.floor();
    ...
    #[inline(always)]
    pub fn emit_i32_i32<const SEED_ID: usize>(
        &mut self,
        to_data1: i32,
        to_data2: i32,
        hash_value: f32,
        ...
        self.tmp_data.push((hash_index, hash_value, 0.0));
    }
}
...
impl TransformExecutor {
    pub fn from_namespace_transform(
        ...
        let namespace_to = ExecutorToNamespace {
            ...
            namespace_seeds: default_seeds(
                namespace_transform
                    .to_namespace
                    .namespace_descriptor
                    .namespace_index as u32,
            ),
            tmp_data: Vec::new(),
        };
        ...
        let te = TransformExecutor {
            ...
            function_executor: Self::create_executor(
                &namespace_transform.function_name,
    ...
    pub fn create_executor(
        ...
        if function_name == "BinnerSqrtPlain" {
            TransformerBinner::create_function(
                &(|x, resolution| x.sqrt() * resolution),
                function_name,
                namespaces_from,
                function_params,
                false,
            ...
            TransformerLogRatioBinner::create_function(
                function_name,
                namespaces_from,
                function_params,
                false,
            ...
            return Err(Box::new(IOError::new(
                ErrorKind::Other,
                format!("Unknown transformer function: {}", function_name),
            )));
        }
    }
}
