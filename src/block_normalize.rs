...
pub fn new_normalize_layer_block(  bg: &mut graph::BlockGraph, 
...
//            let avg = sum / self.num_inputs as f32;
            ...
            for i in 0..self.num_inputs {
                *pb.tape.get_unchecked_mut(self.output_offset + i) = (*pb.tape.get_unchecked(self.input_offset + i) - mean) * variance_inv;
            }
            block_helpers::forward_backward(further_blocks, fb, pb, update);
                
            if update {
                for i in 0..self.num_inputs {
                    *pb.tape.get_unchecked_mut (self.input_offset + i) = *pb.tape.get_unchecked_mut(self.output_offset + i) * variance_inv;
                }
...
*/
            ...
            for i in 0..self.num_inputs {
                *pb.tape.get_unchecked_mut(self.output_offset + i) = *pb.tape.get_unchecked(self.input_offset + i) * var3;
            }
...
pub fn new_stop_block(  bg: &mut graph::BlockGraph, 
                        mi: &model_instance::ModelInstance,
                        input: graph::BlockPtrOutput
                        ) -> Result<graph::BlockPtrOutput, Box<dyn Error>> {    
    let num_inputs = bg.get_num_output_values(vec![&input]);
