...
fn main2() -> Result<(), Box<dyn Error>> {
    ...
    // Where will we be putting predictions (if at all)
    ...
    // we want heap-allocated objects here
    ...
    if cl.is_present("daemon") {
        ...
        let (mut mi2, vw2, re_fixed) =
            persistence::new_regressor_from_filename(filename, true, &cl)?;
        ...
        if let Some(filename) = cl.value_of("initial_regressor") {
            ...
            println!("WARNING: Command line model parameters will be ignored");
            let (mi2, vw2, re2) =
                persistence::new_regressor_from_filename(filename, testonly, &cl)?;
            mi = mi2;
            vw = vw2;
            re = re2;
        } else {
