...
macro_rules! assert_epsilon {
    ($x:expr, $y:expr) => {
        if !($x - $y < 0.0000001 || $y - $x < 0.0000001) {
            panic!();
        }
    };
}
...
pub fn read_weights_from_buf<L: OptimizerTrait>(
    weights: &mut Vec<WeightAndOptimizerData<L>>,
...
pub fn write_weights_to_buf<L: OptimizerTrait>(
    weights: &Vec<WeightAndOptimizerData<L>>,
...
pub fn slearn<'a>(
    block_run: &mut Box<dyn BlockTrait>,
    block_loss_function: &mut Box<dyn BlockTrait>,
    fb: &feature_buffer::FeatureBuffer,
    update: bool,
) -> f32 {
    unsafe {
        let block_loss_function: Box<dyn BlockTrait> =
            mem::transmute(&*block_loss_function.deref().deref());
        ...
        let (prediction_probability, general_gradient) =
            block_run.forward_backward(further_blocks, 0.0, fb, update);
