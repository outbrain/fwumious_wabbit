...
impl Regressor {
    pub fn new_without_weights<L: optimizer::OptimizerTrait + 'static>(
        mi: &model_instance::ModelInstance,
    ) -> Regressor {
    ...
    pub fn new<L: optimizer::OptimizerTrait + 'static>(
        mi: &model_instance::ModelInstance,
    ) -> Regressor {
    ...
    pub fn learn(&mut self, fb: &feature_buffer::FeatureBuffer, update: bool) -> f32 {
        ...
        let (prediction_probability, general_gradient) =
            current[0].forward_backward(further_blocks, 0.0, fb, update);
